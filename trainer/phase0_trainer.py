        ...
        for epoch in pbar:
            if self.use_train:
                ...
                if not self.use_pretrain and epoch % 50 == 0:
                    self.save_checkpoint(self.model, f'Phase0_{self.model_name}_temp.pt')
    ...
    def train_epoch(self):
        ...
        for epoch in pbar:
            if self.use_train:
                ...
                if not self.use_pretrain and epoch % 50 == 0:
                    self.save_checkpoint(self.model, f'{self.model_name}_{self.model_name}_temp.pt')

            if self.use_valid:
                self.valid_epoch(pbar, epoch)

        if not self.use_pretrain:
            file_name = f'{self.model_name}_{self.model_name}_b{self.train_batch_size}_lr{self.lr}_{self.epochs}.pt'
            self.save_checkpoint(self.model, file_name)
